...
/*   Updated: 2025/11/28 15:52:30 by ckrasniq         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	set_tile_size(int cols, int rows, int window_w, int window_h)
{
	if (cols <= 0 || rows <= 0)
		return (1);
	int tile_w = window_w / cols;
	int tile_h = window_h / rows;
	if (tile_w < tile_h)
		return (tile_w);
	return (tile_h);
}
...
int	setup_game_variables(t_game *g, t_map_data *mdata, t_player player)
{
	g->width = 1080;
	g->height = 720;
	g->tile_size = set_tile_size(mdata->map_cols, mdata->map_rows, g->width, g->height);
	if (g->tile_size < 1)
		g->tile_size = 1;
	int prow = player.py;
	int pcol = player.px;
